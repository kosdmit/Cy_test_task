...
class TestRegistrationPage:  # TODO: Group by test objects
    def setup(self):
    ...
    def test_radio_buttons_with_valid_data(self, selector, data_set=valid_data_sets['RU']):
        self.send_data(data_set)
    ...
    def test_with_nums_in_non_num_fields(
        ...
        # Check submit button is not active
        submit_button = self.browser.find_element(By.CSS_SELECTOR, selectors['submit_buttons']['submit'])
        assert submit_button.is_enabled() is False


    # @pytest.mark.xfail  # Expected a message about missing required field
    ...
    invalid_data_tests: tuple[str] = ('value_longer_max_length', 'sql_injection')
